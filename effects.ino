...
void flagsRoutine(int mode)
{
  ...
  //generate new line  
  if(yShift > 1 || yShift < -1) direct = -direct;
  if(flagDuration < 150) {
    renderWRWflag();
  } else if(flagDuration < 300) {
    renderBYflag();
  } else if(flagDuration < 450) {
    renderUAflag();
  } else if(flagDuration < 600) {
    renderRUflag();
  } else {
    flagDuration = 0;
    thisMode++;
  ...
  if(yShift > 0) {
    for (byte y = 0; y < yShift; y++) {
      drawPixelXY(0, y, CRGB::Black);
    }
  }
  if(yShift < 0) {
    for (byte y = HEIGHT+yShift-1; y < HEIGHT; y++) {
      drawPixelXY(0, y, CRGB::Black);
    }
  }
}
...
bool renderBYflag() {
  for (byte y = yShift+1; y < 4+yShift; y++) {
    drawPixelXY(0, y, CRGB::Green);
  }
  for (byte y = 4+yShift; y < HEIGHT+yShift-1; y++) {
      drawPixelXY(0, y, CRGB::Red);
  }
  return true;
}

bool renderWRWflag() {
  for (byte y = yShift+1; y < 3+yShift; y++) {
    drawPixelXY(0, y, CRGB::White);
  }
  for (byte y = 3+yShift; y < 7+yShift; y++) {
    drawPixelXY(0, y, CRGB::Red);
  }
  for (byte y = 7+yShift; y < HEIGHT+yShift-1; y++) {
      drawPixelXY(0, y, CRGB::White);
  }
}

bool renderUAflag() {
  if(flagDuration > 450 || flagDuration < 299) return false;
  for (byte y = yShift+1; y < 5+yShift; y++) {
    drawPixelXY(0, y, CRGB::Yellow);
  }
  for (byte y = 5+yShift; y < HEIGHT+yShift-1; y++) {
      drawPixelXY(0, y, CRGB::Blue);
  }
}
...
// *********** радуга дигональная ***********
void rainbowDiagonalRoutine(int mode) {
  modeCode = mode;
  hue += 3;
  for (byte x = 0; x < WIDTH; x++) {
    for (byte y = 0; y < HEIGHT; y++) {
